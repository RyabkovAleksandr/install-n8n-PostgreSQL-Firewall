...
# Скрипт для развертывания n8n на Ubuntu 24.04 VPS с PostgreSQL и портом 5670
    ...
    if [ -f /etc/os-release ]; then
        ...
        case $VERSION_ID in
            ...
            *)
                ...
                echo "Поддерживаемая версия: Ubuntu 24.04 LTS"
                exit 1
                ;;
        esac
    else
...
main() {
    ...
    print_header "Установка n8n на Ubuntu 24.04 VPS (PostgreSQL, порт 5670)"
    print_info "Скрипт поддерживает только Ubuntu 24.04 LTS"
    echo
...
case "${1:-}" in
    --help|-h)
        echo "Скрипт для развертывания n8n на Ubuntu 24.04 VPS (PostgreSQL, порт 5670)"
        echo ""
    ...
    --version|-v)
        ...
        echo "Поддерживает Ubuntu 24.04 LTS"
        exit 0
        ;;
    --update)
